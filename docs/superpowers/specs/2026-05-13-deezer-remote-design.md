...
- [ ] **`doctor` is blind to host-firewall drops on Linux.** Surfaced 2026-05-14 during the manual smoke test above: with `firewalld` active and the default `public` zone, inbound 8080 is silently dropped, yet `doctor`'s "LAN reachable from self" check still passes — its TCP probe originates on the same host, and `firewalld` does not filter local-origin traffic. The structural fix: when `systemctl is-active firewalld` returns active, look up the active wlan/eth NM connection's zone (e.g. `nmcli -g connection.zone connection show <name>`) and warn if the configured `--port` is not permitted in that zone. A real external-reachability test would require an out-of-host vantage point (have the user curl from the phone, or bind-test from a non-loopback address with `SO_BINDTODEVICE`). Background and the per-SSID zone recipe: `docs/solutions/integration-issues/phone-cant-reach-laptop-virtual-bridges-and-firewalld-2026-05-14.md`. Commit `d20dfb7` already filtered host-internal virtual bridges from `LANAddrs` (necessary half-of-the-fix); the firewall blindness remains.
